<template>
  ...
  <div class="container">
    ...
    <div class="ui-button add" v-if="user.role == 'SYSTEM_ADMIN'">
      <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#AddSystemAdminModal">Add system admin</button>
    </div>

    <div class="modal fade" id="AddSystemAdminModal" tabindex="-1" aria-labelledby="AddSystemAdminModal" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="AddSystemAdminModal">Add new system admin</h5>
          ...
          <div class="modal-body">
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-primary" v-on:click="addSystemAdmin">ADD</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="row card-group" v-for="n in Math.ceil(systemAdmins.length/4)" :key="n.id">
      <div class="col-3" v-for="systemAdmin in systemAdmins.slice((n-1)*4, n*4)" :key="systemAdmin.id">
        <div class="card">
          <img class="card-img-top" src="../../public/medical-worker-profile-pic.png" alt="Card image cap">
          <div class="card-header"><b>{{ systemAdmin.firstName }} {{ systemAdmin.lastName }}</b></div>
          <div class="card-body">
            <p>Email: {{ systemAdmin.email }}</p>
            <p>Phone number: {{ systemAdmin.phoneNumber }}</p>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    loadSystemAdmins() {
      SystemAdminService.systemAdminsPage()
      .then((response) => {
        this.systemAdmins = response.data.systemAdmins;
    ...
    addSystemAdmin(event) {
      if (this.validate()) {
        SystemAdminService.addSystemAdmin(this.input)
            .then(response => {
              this.$toast.show('System admin successfully added!', {
                type: 'info',
                position: 'top',
                duration: 2000
              });
