<template>
  ...
  <div class="d-flex justify-content-between">
    ...
    <div v-if="role==='PHARMACY_ADMIN'" class="ui-button">
      <button class="btn btn-primary" @click="showHire=true">Hire new pharmacist</button>
    </div>
  </div>
            ...
            <table class="table user-list">
              ...
              <tr v-for="phar in pharmacists" :key="phar.id">
                <td>
                  ...
                  <h6>{{phar.firstName}} {{phar.lastName}}</h6>
                </td>
                ...
                <td>
                  {{ phar.address.name }} {{ phar.address.number }}, {{ phar.address.city }} {{ phar.address.postalCode }}
                </td>
                <td v-if="role==='PHARMACY_ADMIN'" style="width: 20%;">
                  <a href="#" class="table-link" @click="isChart=true; currentPharmacist = phar">
                    <span class="fa-stack">
                      <i class="fa fa-square fa-stack-2x"></i>
                      <i class="fa fa-star fa-stack-1x fa-inverse"></i>
                    </span>
                  </a>
                  <a href="#" class="table-link" @click="appointmentsStats=true; currentPharmacist=phar">
                    <span class="fa-stack">
                      <i class="fa fa-square fa-stack-2x"></i>
                      <i class="fa fa-calendar fa-stack-1x fa-inverse" aria-hidden="true"></i>
                    </span>
                  </a>
                  <a href="#" class="table-link" @click="showAppointments(phar)">
                    <span class="fa-stack">
                      <i class="fa fa-square fa-stack-2x"></i>
                      <i class="fa fa-search-plus fa-stack-1x fa-inverse"></i>
                    </span>
                  </a>
                  <a href="#" class="table-link danger" @click="areYouSure(phar, pharmacyId)">
                    <span class="fa-stack">
                      <i class="fa fa-square fa-stack-2x"></i>
                ...
                <td v-else>
                  <button class="btn btn-secondary" @click="showAppointments(phar)">Set appointment</button>
                </td>
              </tr>
              </tbody>
            </table>
        ...
        <div class="modal-wrapper">
          <div class="modal-container">
            <div class="modal-header">
              <button type="button" class="btn btn-close" data-bs-dismiss="modal" @click="showHire=false; showCalendar=false"></button>
            </div>
            <div class="modal-body" id="modal-kalendar">
              <DefineNewAppointment v-if="showCalendar"  :worker="currentPharmacist" :pharmacy-id="pharmacyId"
                                    :start-work="schedule.start" :end-work="schedule.end"
                                    role="pharmacist" :user-role="currentRole" :user-id="currentUserId"></DefineNewAppointment>
              <HireNewPharmacist v-if="showHire" :pharmacyId="pharmacyId"></HireNewPharmacist>
            </div>
          </div>
        </div>
      </div>
  ...
  <Chart v-if="isChart" v-on:close="isChart=false" ref="chart" type="pharmacist" :pharmacist="currentPharmacist"></Chart>

  <StatsChart v-if="appointmentsStats" type="pharmacist" ref="appointmentsStats"
                         @close="appointmentsStats=false" :pharmacist="currentPharmacist"></StatsChart>
</template>

<script>
import {mapState} from "vuex";
import HireNewPharmacist from "./HireNewPharmacist";
...
export default {
  ...
  components: {DefineNewAppointment, HireNewPharmacist, ConfirmationDialogue, Chart, StatsChart},
  data: function() {
    return {
      pharmacists: [],
      title: "",
  ...
  methods: {
    loadPharmacists() {
      PharmacyService.getEmployedWorkers(this.$route.query.id, 'pharmacist')
        .then((response) => {
    ...
    searchPharmacists() {
      ...
      MedicalWorkerService.searchWorker('pharmacist', this.searchParameters)
      .then((response) => {
    ...
    updatePage(pageNumber) {
      ...
      MedicalWorkerService.updatePageSearchWorker('pharmacist', pageNumber, this.searchParameters)
      .then((response) => {
        this.pharmacists = response.data.workers;
        this.totalPages = response.data.totalPages;
        if (this.pharmacists.length === 0 && this.currentPage > 0)
          this.updatePage(this.currentPage - 1);
      });
    },
    loadSchedule(pharmacistId) {
      MedicalWorkerService.getWorkerSchedule('pharmacist', pharmacistId, this.$route.query.id)
        .then((response) => {
          this.schedule = response.data;
          this.showCalendar = true;
        });
    },
    showAppointments(pharmacist) {
      this.currentRole = this.role;
      this.currentUserId = this.userId;
      this.currentPharmacist = pharmacist;
      this.loadSchedule(this.currentPharmacist.id);
    ...
    areYouSure(pharmacist, pharmacyId) {
      this.$refs.confirmDialogue.show({
        title: "Fire pharmacist "+ pharmacist.firstName + " " + pharmacist.lastName,
        message: "Are you sure you want to fire this pharmacist?",
        okButton: 'Fire'
      }).then((result)=>{
        if(result)
          this.pharmacistDismissal(pharmacist, pharmacyId);
      });
    },
    pharmacistDismissal(pharmacist, pharmacyId) {
      MedicalWorkerService.getFutureAppointments('pharmacist', Number(pharmacist.id), Number(pharmacyId))
      .then((response) => {
        if(response.data.length === 0) {
          MedicalWorkerService.firePharmacist(pharmacist.id, pharmacyId)
          .then((response) => {
            this.loadPharmacists();
            this.$toast.show('Pharmacist \''+ pharmacist.firstName + ' ' + pharmacist.lastName +' \' fired successfully!', {
              type: 'info',
              position: 'top',
              duration: 6000
            });
          ...
          this.$toast.show('Pharmacist can\'t be fired because of future reserved appointments!', {
            type: 'info',
            position: 'top',
            duration: 6000
          });
