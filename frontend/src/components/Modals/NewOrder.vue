<template>
  <popup-modal ref="popup">
    ...
    <div class="container">
      ...
      <div class="ui-button">
        ...
        <input type="datetime-local" class="col-sm-3" id="until-datetime" v-model="deadline">
        <button class="btn btn-success" @click="makeOrder()" style="margin-left: 10px">Make order</button>
  ...
  <NewOrderItem v-if="addItem" v-on:close="addItem=false" :order="order"></NewOrderItem>
</template>
...
export default {
  name: "NewOrder",
  emits: ['close'],
  components: {NewOrderItem, PopupModal},
  data() {
    return {
      addItem: false,
      deadline: null,
      order: {
        orderItems: [], // medicineDTO, amount
  ...
  methods: {
    ...
    makeOrder() {
      ...
      this.order.deadline = deadline.getTime();
      this.order.pharmacyAdminId = this.userId;
      this.order.pharmacyId = parseInt(this.order.pharmacyId);
      console.log(this.order);
      this.makeReq();
    },
    makeReq() {
      PharmacyService.newOrder(this.order)
      .then((response) => {
        this.close();
        this.toast("New order successfully made!");
      });
    }
  },
  computed: {
