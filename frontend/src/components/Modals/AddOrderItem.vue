...
export default {
  ...
  methods: {
    ...
    newItem() {
      PharmacyService.addOrderItem(this.order.id, this.itemDTO)
      .then((response) => {
        if(response.data) {
          this.$emit('reloadItems');
          this.toast('Medicine \'' + this.medicineShow.name + '\' added successfully!');
        } else {
          this.toast("Can't add items, because this order have offers!");
        }
      });
    }
  }
